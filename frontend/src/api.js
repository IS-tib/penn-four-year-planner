...
export const api = {
  ...
  deletePlan: (token, planId) =>
    request(`/api/plans/${planId}`, { method: "DELETE", token }),
  ...
  autofill: (token, planId) =>
    request(`/api/plans/${planId}/autofill`, { method: "POST", token }),
  ...
  replacePlacements: (token, planId, placements) =>
    request(`/api/plans/${planId}/placements`, {
      method: "PUT",
      body: { placements },
      token,
    }),

  eligible: (token, planId, { termIndex, category, excludePlaceholders } = {}) => {
    const params = new URLSearchParams({ term_index: String(termIndex) });
    if (category) params.set("category", category);
    if (excludePlaceholders) params.set("exclude_placeholders", "true");
    return request(`/api/plans/${planId}/eligible?${params}`, { token });
  },

  share: (token, planId) =>
    request(`/api/plans/${planId}/share`, { method: "POST", token }),
  unshare: (token, planId) =>
    request(`/api/plans/${planId}/share`, { method: "DELETE", token }),
...
export async function downloadCsv(token, planId, filename) {
  ...
  link.click();
  link.remove();
  // Revoking immediately can cancel the download in some browsers, so give it
  // a moment first.
